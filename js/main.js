document.addEventListener('DOMContentLoaded', function () {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function (event) {
            ...
            emailjs
                .sendForm('service_y9eokla', 'template_07bfz5o', this)
                .then(function () {
                    // Success message
    ...
    if (servicesTrack) {
        ...
        fetch('includes/services.html')
            ...
            .then(html => {
                ...
                console.log(
                    `Found ${mobileTiles.length} mobile service tiles to add click events to`
                );
            ...
            .catch(error => {
                ...
                servicesTrack.innerHTML =
                    '<p class="services-error">Failed to load services content.</p>';
            });
    } else {
    ...
    function setupNaturalScrolling(container, options = {}) {
            ...
            if (Math.abs(deltaX) > Math.abs(deltaY) * 1.5) {
                ...
                if (
                    ...
                    const resistedDeltaX =
                        deltaX * (isTimeline ? config.resistance * 1.8 : config.resistance);
                    ...
                    if (elapsed > 0) {
                        ...
                        velocityX =
                            (1 - velocityWeight) * velocityX + velocityWeight * (deltaX / elapsed);
                    }
                }
        ...
        function onTouchEnd() {
            ...
            if (isOverscrolling && overscrollAmount !== 0) {
                ...
                const delay =
                    Math.abs(overscrollAmount) > container.offsetWidth * 0.1
                        ? Math.min(Math.abs(overscrollAmount), maxDelay)
                        : 0;
        ...
        function startMomentumTracking(initialVelocity) {
            ...
            const scroll = () => {
                ...
                if (
                    ...
                    const maxOverscroll =
                        container.offsetWidth * (isMobile ? 0.07 : 0.04) + momentumFactor * 0.06;
                    overscrollAmount = Math.max(
                        Math.min(overscrollAmount, maxOverscroll),
                        -maxOverscroll
                    );
    ...
    function handleServiceTileClick(event) {
        ...
        const modalId = this.getAttribute('data-modal');
        console.log(
            `Opening modal from ${window.innerWidth <= 768 ? 'mobile' : 'desktop'} view: ${modalId}`
        );
        openModal(modalId);
    }
